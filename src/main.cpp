...
// Copyright (c) 2013 The Gaeacoin developers
...
static const unsigned int timeGenesisBlock = 1459764104;
...
const string strMessageMagic = "Gaeacoin Signed Message:\n";
...
static const int64 nTargetTimespan = 262800 * 600; // readjust difficulty once per hour
static const int64 nTargetSpacing = 60 * 10; // 30 seconds one block
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "already in already databases we Gaeacoin from USA with reindexing.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GaeacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GaeacoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("GaeacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Gaeacoin-miner");
    ...
    try { loop {
        ...
        printf("Running GaeacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("GaeacoinMiner terminated\n");
        throw;
    }
}
